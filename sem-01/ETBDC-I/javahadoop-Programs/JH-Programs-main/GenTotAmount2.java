...
// THIS PROGRAM DOES "3.A." and maximum in a day
...
public class GenTotAmount2 {
	...
	public static class GenTotAmountMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			context.write(new Text(row[1]), new IntWritable(Integer.parseInt(row[2]) * Integer.parseInt(row[3])));
	...
	public static class GenMaxAmountMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
		private static final Logger logger_m = Logger.getLogger(GenTotAmountMapper.class);
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			String line = value.toString().trim();
			String[] row = line.split(",");	
	...
	public static class GenMaxAmountReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
		private IntWritable result = new IntWritable();
		private static final Logger logger_r = Logger.getLogger(GenTotAmountReducer.class);
		...
		public void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
			...
			result.set(maxval);
			System.out.println("In reducer: Processing key: " + key.toString() + ", value: " + maxval);
			//~ logger_r.info("In reducer: Processing key: " + key.toString() + ", value: " + sum);

			//~ System.out.println("sum = " + sum);
			//~ context.write(new Text("Maximum products sold in a day are:"), new IntWritable(0));
			context.write(key, result);
		}	
	}
	...
	public static void main(String[] args) throws Exception {
        ...
        if (job.waitForCompletion(true)) {
			Job job2 = Job.getInstance(conf, "get max amount");
			job2.setJarByClass(GenTotAmount.class);
			job2.setMapperClass(GenMaxAmountMapper.class);
			//~ job.setCombinerClass(GenTotAmountReducer.class);
			job2.setReducerClass(GenMaxAmountReducer.class);
