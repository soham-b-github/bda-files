...
# -----------------------------------------------------------------------
barplot(table(Region))                  # vertical bar diagram
help("barplot")
barplot(table(Region),
        xlab = 'Region',ylab = 'Number of people',
        ylim = c(0,08),col='blue')        # vertical bd customized
title(main='Region Frequency Distribution',sub='Vertical bar diagram')
box()
# -----------------------------------------------------------------------
# 2.a.(ii) horizontal bar diagram
# -----------------------------------------------------------------------
barplot(table(Region),horiz = TRUE)     # horizontal bar diagram
barplot(table(Region),horiz = TRUE,
        ylab = 'Region', xlab = 'Number of people',
        xlim = c(0,9),col='blue')         # horizontal bd customized
title(main='Region Frequency Distribution',sub='Horizontal bar diagram')
box()
# -----------------------------------------------------------------------
# 2.a.(iii) divided bar diagram
# -----------------------------------------------------------------------
data=matrix(table(Region), nrow=5)
data
...
title(main='Region percentages',sub='Divided Bar Diagram')
box()
# -----------------------------------------------------------------------
# 2.a.(iv) pie chart
# -----------------------------------------------------------------------
pie(table(Region))                      # pie chart
pie(table(Region),col=c('red','white','orange','green','blue'),clockwise = F, 
    init.angle = 0,cex=1.2)             # pie chart customized
title(main='Region percentages',sub='Pie Chart')
box()
#########################################################################
...
# -----------------------------------------------------------------------
table(siblings)
plot(seq(0,4,1),cumsum(table(siblings)),type='s')
plot(seq(0,4,1),cumsum(table(siblings)),type='s',col='blue',
...
barplot(table(siblings),xlab = 'Number of Siblings',
        ...
        border = 'red',density = 1)      # vertical bd customized
help(abline)
abline(h=c(seq(0,16,4)), lty = 3, lwd = .5, col = 'black')         # adding horizontal axes in bd
...
# 2.c.(iv) Box Plot
# -----------------------------------------------------------------------
b<-boxplot(baby_wt,col=c("green"),border="black",
        horizontal=T,notch=F,range = 1.5)
...
expenditure
barplot(expenditure,main = "total expenditure",names.arg = countries,xlab = "countries",
...
# -----------------------------------------------------------------------
library(ggplot2)
dRel=as.data.frame(table(Region))
pie = ggplot(dRel,aes(factor(Region),y=as.numeric(Freq),fill=Region)) +
  geom_bar(width =0.5,stat = "identity")+xlab("Region")+
...
# -----------------------------------------------------------------------
bp<- ggplot(dRel, aes(factor(Region),y=as.numeric(Freq),fill=Region))+
  geom_bar(width = 1,stat="identity")+xlab("Region")+
