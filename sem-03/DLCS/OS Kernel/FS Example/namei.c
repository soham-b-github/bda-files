		  ...
		  struct dentry *dentry /* NULL during fast_commit recovery */)
{
	...
	/* Place this at the beginning of the __ext4_unlink function */

/*
 * ===================================================================
 * FINAL VERSION: Prevent deletion of IMMUTABLE .log files,
 * using the correct d_name argument.
 */
{
    // The inode of the file being deleted is passed in directly.
    // The d_name qstr is the reliable source for the filename.
    if (inode && d_name && d_name->name) {
        // Check if the immutable flag is set on this inode
        // if (inode->i_flags & EXT4_IMMUTABLE_FL) {
            const char *filename = d_name->name;
            const char *suffix = ".logx1y2";
            size_t len_filename = strlen(filename);
            size_t len_suffix = strlen(suffix);

            if (len_filename >= len_suffix) {
                if (strncmp(filename + len_filename - len_suffix, suffix, len_suffix) == 0) {
                    // All conditions are met: immutable AND ends in .log. Deny deletion.
                    printk(KERN_WARNING "EXT4_PROTECT: Denying deletion of protected log file: %s\n",
                           filename);
                    return -EPERM;
                }
            }
