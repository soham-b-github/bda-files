...
SurvObject = Surv(myCredit$Duration,CensorData$Status)
...
# Build the coxph model
ResultSP = coxph(SurvObject ~ Age+Amount+InstallmentRatePercentage+
                   NumberExistingCredits+NumberPeopleMaintenance, data=myCredit)     
summary(ResultSP)
...
ResultSP = coxph(SurvObject ~ Amount + InstallmentRatePercentage, data=myCredit)     
summary(ResultSP)
